...
function buildErrorResponse(
  ...
  return NextResponse.json({ success: false, error: message, code, response: message, state }, { status });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (e instanceof InvalidSessionTokenError) {
      const res = buildErrorResponse("Token inválido o expirado", 401, "neutral", "INVALID_SESSION_TOKEN");
  ...
  if (!message) {
    return buildErrorResponse("Necesito un mensaje para ayudarte.", 400, "neutral", "EMPTY_MESSAGE");
  }
