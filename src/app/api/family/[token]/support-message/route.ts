...
export async function POST(
  ...
  try {
    ...
    if (deliverNow) {
      void deliverSupportMessage(msg.id).catch((err) =>
        logError("FAMILY", err instanceof Error ? err : new Error(String(err)), { context: "deliverSupportMessage", messageId: msg.id }),
      );
    }

    return NextResponse.json({ ok: true, message: msg }, { status: 201 });
  } catch (err) {
    logError("FAMILY", err instanceof Error ? err : new Error(String(err)), { route: "/api/family/[token]/support-message", method: "POST" });
...
export async function GET(
  ...
  try {
    ...
    logError("FAMILY", err instanceof Error ? err : new Error(String(err)), { route: "/api/family/[token]/support-message", method: "GET" });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
